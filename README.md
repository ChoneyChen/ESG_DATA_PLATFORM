...
> 本文件是整个仓库唯一的项目级事实入口，面向项目成员和新的 Codex 对话。
> 内容以 2026-09-10 的实际代码、配置和目录为准。若历史聊天、旧方案或演示材料与本文件冲突，以代码合同和本文件为准。
...
ESRS E1-5 和 E1-6 已发布 draft `1.2.0` 标准包，强化指标语义和核算方法说明；
...
当前目录合同是 `targeted-result-bundle-v1`，结果合同是 `targeted-extraction-result-v1`。Core 随任务快照固定：E2-4 使用 `extractesg.core@1.0.0`，E1-5/E1-6 使用向后兼容的 `extractesg.core@1.1.0`；E1 当前运行时标准包为 `1.2.0`。
...
E1-5、E1-6 的 `1.0.0` 已完成首轮紫金/越秀真实任务并暴露检索假阴性、复杂能源表语义混入和 region 标量合同问题；`1.1.0` 是历史工程升级版本，当前运行时发布为 `1.2.0`，仍处于 `draft`，尚未获得业务批准。
...
下一步由用户用 `1.2.0` 复跑紫金/越秀 E1-5_02/05/12 与 E1-6_07/08。验证顺序仍为 Query/FactPattern、Evidence Selection、实际模型输入、多行输出、单位/期间/实体和证据绑定；发现问题时优先修概念、模型上下文或通用合同，禁止加入 `E1-5_xx`、`E1-6_xx` 条款号分支。

直接披露与计算继续解耦。报告直接给出的总量、占比或强度照常抽取；未披露时不在语义填表阶段临时计算。`derivations.json` 声明计算关系，执行计算属于后续数据处理模块。

## 16. 已知缺口

1. E1-5、E1-6 已完成首轮真实报告诊断和 `1.2.0` 工程升级，但尚未通过复跑验收与业务批准；
