...
旧 revision 若停在 `required_review_targets_incomplete_after_guard_confirmation`，恢复入口会在
新子版本内重新判断可重试性，并保留已经接受的局部确认。跨页模型确认后，编排器在唯一几何
关系可确定时补齐横向关联；未覆盖的目标进入下一轮局部反馈，不再因本轮已有部分确认就立即
锁成永久系统阻塞。历史包本身不原地改写。04 复核工作清单对可重试项直接提供“重跑此项”，
按该行所属 IR revision 和 Task ID 创建子版本并进入统一队列；任务结束后刷新最新分支。
...
Markdown fallback 的单行、纯文字竖线列表保留为可搜索原文，不直接升成正式表格。已保存
的缺坐标单行候选可在局部修复中判为普通文字并无损退役；复核任务只有在原先的几何问题
真正消失后才关闭。Validator 输出缺坐标表格的对象 ID 和 PDF 页码，04 页把复核任务及
仍阻塞的验证问题一同列出，并可定位对象、预填局部修复目标。页面覆盖复核的上下文以
对象索引和坐标为主，避免重复发送整页每个对象的完整结构。页级任务只修可定位文字，
图表交给其对象任务；存在原生文字覆盖异常的大型视觉候选会带着页级线索进入对象复核。
模型仍负责判断可见内容及目标归属，JSON 协议重试会带着具体解析反馈进行纠正。
